...
class ExternalCallServiceImplTest {
  ...
  private static final String QRCODE = "UEFHT1BBfDAwMnwwMDAwMDAwMDAwMDAwMDAwMDB8Nzc3Nzc3Nzc3Nzd8OTk5OQ";
  private static final String PA_TAX_CODE = "20000000000";
  private static final String NOTICE_NUMBER = "302051234567890125";
  ...
  void setUp() throws NoSuchFieldException, IllegalAccessException {
    ...
    // Create spy once here
    spyExternalCallServiceImpl =
        Mockito.spy(new ExternalCallServiceImpl(
  ...
  void executeExternalCall_ActivateByQrCode_Patch() throws JsonProcessingException {
    ...
    testVariables.put("PathParams", Map.of("qrCode", QRCODE));
    testVariables.put(
        "body", objectMapper.writeValueAsString(PaymentTestData.getActivatePaymentRequest()));
  ...
  void executeExternalCall_sendPaymentOutcome() throws JsonProcessingException {
    ...
    testVariables.put("method", "PATCH");
    testVariables.put(
        "body", objectMapper.writeValueAsString(PaymentTestData.getClosePaymentRequest(true)));

    when(paymentService.sendPaymentOutcome(any(), any(), any()))
  ...
  void executeExternalCallTestClientException()
    ...
    doReturn(new URI("http://mil-base-path/endpoint/params"))
        .when(spyExternalCallServiceImpl)
        .prepareUri(any(), any());
  ...
  void prepareUriTestOK() {
    ...
    URI resultIdPay =
        spyExternalCallServiceImpl.prepareUri(configuration, FlowValues.IDPAY.getValue());
    assertEquals("http://idpay-base-pathendpoint", resultIdPay.toString());

    URI resultAuth =
        spyExternalCallServiceImpl.prepareUri(configuration, FlowValues.AUTH.getValue());
  ...
  @Test
  void executeExternalCall_CreateTransaction_Post() throws JsonProcessingException {
    // Prepare test variables for MIL flow with reporting service endpoint
    testVariables.put(RequiredProcessVariables.FLOW.getEngineValue(), FlowValues.MIL.getValue());
    testVariables.put("url", "/transactions");
    testVariables.put("method", "POST");
    testVariables.put(
        "body", objectMapper.writeValueAsString(PaymentTestData.getPagoPaTransactionRequest()));
    testVariables.put("PathParams", new HashMap<>());

    PagopaTransactionsDto mockResponse = new PagopaTransactionsDto();
    mockResponse.setTransactionId("txn_123");
    mockResponse.setStatus("C");
    when(reportingService.createTransaction(any()))
        .thenReturn(ResponseEntity.status(HttpStatus.CREATED).body(mockResponse));

    ResponseEntity<String> result = spyExternalCallServiceImpl.executeExternalCall(testVariables);

    verify(reportingService).createTransaction(any());
  ...
  @Test
  void executeExternalCall_CreateTransaction_Exception() throws JsonProcessingException {
    // Prepare test variables for MIL flow with reporting service endpoint that will throw exception
    testVariables.put(RequiredProcessVariables.FLOW.getEngineValue(), FlowValues.MIL.getValue());
    testVariables.put("url", "/transactions");
    testVariables.put("method", "POST");
    testVariables.put(
