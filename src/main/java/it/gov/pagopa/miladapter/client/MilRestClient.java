...
public class MilRestClient {
  ...
  public MilRestClient(
      WebClient.Builder webClientBuilder, @Value("${mil-rest-api.base-url}") String baseUrl) {
  ...
  public Mono<AcquirerConfiguration> getPspConfiguration(String authorization, String acquirerId) {
    ...
    return webClient
        ...
        .doOnSuccess(
            config ->
                log.debug(
                    "Successfully retrieved PSP configuration for acquirerId: {}", acquirerId))
        .doOnError(
            error ->
                log.error(
                    "Error retrieving PSP configuration for acquirerId: {}", acquirerId, error));
  }
}
